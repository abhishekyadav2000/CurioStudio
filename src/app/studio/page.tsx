  ...
  return (
    <AppShell>
      <div className="max-w-3xl mx-auto p-4 lg:p-8">
          <h2 className="text-2xl font-bold mb-1">Studio</h2>
          <p className="text-sm text-muted mb-6">Teleprompter and presentation — pick a ready project</p>

          {ready.length === 0 ? (
            <div className="text-center py-16 rounded-2xl border border-dashed border-border">
              <Clapperboard className="w-10 h-10 text-muted mx-auto mb-3 opacity-50" />
              <p className="text-muted mb-4">No scripts ready yet.</p>
              <Link href="/queue" className="text-accent hover:underline text-sm">
                Check Queue for pipeline progress →
              </Link>
            </div>
          ) : (
              ...
              {ready.map((p) => (
                ...
                >
                  <div className="min-w-0">
                    <h4 className="font-semibold group-hover:text-accent truncate">{p.name}</h4>
                    <p className="text-xs text-muted truncate">{p.content?.youtubeTitle}</p>
                  </div>
                  <span className="flex items-center gap-1 text-xs text-accent shrink-0 ml-2">
